...
def detect_double_lines(points, warped_mask, threshold_distance=15, min_points_per_line=5):
    """
    Detect double lines by clustering points into two distinct groups based on x-coordinates
    """
    if len(points) < min_points_per_line * 2:
        return False

    # Extract x-coordinates from points
    x_coords = [p[1] for p in points]

    # Use a simple clustering approach to find if there are two distinct x-groups
    sorted_x = sorted(x_coords)

    # Try to find a gap that separates the two lines
    gaps = []
    ...
    # If the largest gap is small relative to the overall spread, we might have overlapping lines
    overall_spread = sorted_x[-1] - sorted_x[0]

    # Check if we can split into two clusters with significant separation
    if max_gap > threshold_distance and overall_spread > threshold_distance:
        left_group = sorted_x[:max_gap_idx + 1]
        right_group = sorted_x[max_gap_idx + 1:]

        # Both groups should have enough points to be considered valid lines
        if len(left_group) >= min_points_per_line and len(right_group) >= min_points_per_line:
            # Calculate variance within each group to ensure they're well-defined
            left_variance = np.var(left_group) if len(left_group) > 1 else 0
            right_variance = np.var(right_group) if len(right_group) > 1 else 0

            # If variances are reasonable (not too high), consider it a double line
            if left_variance < threshold_distance ** 2 and right_variance < threshold_distance ** 2:
                return True

    # Alternative method: Try K-means-like clustering
    # Sort points by x-coordinate
    sorted_points = sorted(points, key=lambda p: p[1])

    # Try to divide into two groups based on median x
    mid_idx = len(sorted_points) // 2
    if mid_idx == 0:
        return False

    left_x_coords = [p[1] for p in sorted_points[:mid_idx]]
    right_x_coords = [p[1] for p in sorted_points[mid_idx:]]

    if len(left_x_coords) < min_points_per_line or len(right_x_coords) < min_points_per_line:
        return False

    # Check if the groups are well-separated
    left_mean = np.mean(left_x_coords)
    right_mean = np.mean(right_x_coords)

    if right_mean - left_mean > threshold_distance:
        # Check internal consistency of each group
        left_std = np.std(left_x_coords)
        right_std = np.std(right_x_coords)

        if left_std < threshold_distance / 2 and right_std < threshold_distance / 2:
            return True
...
def finalDisplay(image_undistorted, image_bin, image_warped_colored, display_image, image_overlay, left_curvature,
    ...
    # Update lane type display to show "Double Yellow" when appropriate
    left_display = left_lane_type
    if left_lane_type == "Yellow" and left_line_pattern != "Unknown":
        # Check if this is a double yellow line by analyzing the points
        left_display = "Double Yellow"  # Simplified - you might want more complex logic here

    right_display = right_lane_type
    if right_lane_type == "Yellow" and right_line_pattern != "Unknown":
        right_display = "Double Yellow"  # Simplified - you might want more complex logic here

    lane_type_below_turn = f"Left: {left_display}({left_confidence:.2f})-{left_line_pattern}, Right: {right_display}({right_confidence:.2f})-{right_line_pattern}"
...
def process_frame_for_moviepy(frame_bgr):
    ...
    PerspectiveConfigFile = 'perspective_points1.yaml'
    try:
    ...
    # 先用基础方法提取车道线
    white_mask_base, yellow_mask_base = extractWhiteYellow_separate(image_roi)
    image_bin_base = cv2.bitwise_or(white_mask_base, yellow_mask_base)

    # 获取基础处理的 warped 版本用于双线检测
    image_warped_binary_base, H_left, H_right = getWarpedBinary(image_bin_base, PerspectiveConfigFile)
    warped_white_base, _, _ = getWarpedBinary(white_mask_base, PerspectiveConfigFile)
    warped_yellow_base, _, _ = getWarpedBinary(yellow_mask_base, PerspectiveConfigFile)

    # 获取基础处理的点用于双线检测
    left_indexes_base, right_indexes_base = getLeftRightPoints_enhanced(image_warped_binary_base)

    # 检测双线情况
    left_has_double = False
    right_has_double = False

    if len(left_indexes_base) > 0:
        if detect_double_lines(left_indexes_base, image_warped_binary_base):
            left_has_double = True

    if len(right_indexes_base) > 0:
        if detect_double_lines(right_indexes_base, image_warped_binary_base):
            right_has_double = True

    # 根据检测结果决定如何处理
    white_mask, yellow_mask = white_mask_base.copy(), yellow_mask_base.copy()

    # 如果左边检测到双实线，右边使用 extractWhiteYellow_separate 处理
    if left_has_double and not right_has_double:
        # 提取图像右半部分，应用 extractWhiteYellow_separate
        h_roi, w_roi = image_roi.shape[:2]
        right_half = image_roi[:, w_roi // 2:]

        white_mask_right, yellow_mask_right = extractWhiteYellow_separate(right_half)

        # 将处理后的右半部分合并回原图
        white_mask[:, w_roi // 2:] = white_mask_right
        yellow_mask[:, w_roi // 2:] = yellow_mask_right

    # 如果右边检测到双实线，左边使用 extractWhiteYellow_separate 处理
    elif right_has_double and not left_has_double:
        # 提取图像左半部分，应用 extractWhiteYellow_separate
        h_roi, w_roi = image_roi.shape[:2]
        left_half = image_roi[:, :w_roi // 2]

        white_mask_left, yellow_mask_left = extractWhiteYellow_separate(left_half)

        # 将处理后的左半部分合并回原图
        white_mask[:, :w_roi // 2] = white_mask_left
        yellow_mask[:, :w_roi // 2] = yellow_mask_left

    # 如果两边都检测到或都没检测到，则使用原始处理结果
    # (保持 white_mask, yellow_mask 为 base 版本)
    ...
    # Check for double yellow lines on the left
    if len(left_indexes) > 0:
        ...
        # Check if this is a double yellow line
        if left_type == 'Yellow' and len(left_indexes) > 10:  # More points needed for reliable detection
            if detect_double_lines(left_indexes, image_warped_binary):
                current_left_lane_type = 'Double Yellow'
                current_type_confidence_left = max(current_type_confidence_left, 0.9)
    ...
    # Check for double yellow lines on the right
    if len(right_indexes) > 0:
        ...
        # Check if this is a double yellow line
        if right_type == 'Yellow' and len(right_indexes) > 10:  # More points needed for reliable detection
            if detect_double_lines(right_indexes, image_warped_binary):
                current_right_lane_type = 'Double Yellow'
                current_type_confidence_right = max(current_type_confidence_right, 0.9)
    ...
    if left_lane_detected and draw_points_left is not None:
        try:
            ...
            if current_left_lane_type == 'Double Yellow':
                color = (0, 255, 255)  # Yellow color for double yellow
            elif current_left_lane_type == 'Yellow':
                color = (0, 255, 255)
    ...
    if right_lane_detected and draw_points_right is not None:
        try:
            ...
            if current_right_lane_type == 'Double Yellow':
                color = (0, 255, 255)  # Yellow color for double yellow
            elif current_right_lane_type == 'Yellow':
                color = (0, 255, 255)
